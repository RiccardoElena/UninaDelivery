...
public class OrdiniController implements Initializable {
   ...
   */
  @FXML
  private TableColumn<Order, String> clientColumn = new
  ...
  /**
   * On exitButton click, it shows a confirmation alert.
   * If the user confirms, it closes the application.
  ...
  private void applyFadeTransition(final Stage stage, final double fromValue,
                                  final double toValue,
                                  final EventHandler<ActionEvent> onFinish) {
    FadeTransition fadeTransition =
        new FadeTransition(Duration.seconds(FADE_OUT_DURATION),
  ...
  public final void minimizeButtonAction(final ActionEvent event) {
    ...
    // Crea un effetto di fade out
    applyFadeTransition(stage, 1.0, 0.0,
                        e -> {
  ...
  public final void initialize(final URL location,
    ...
    orderNumberColumn.setCellValueFactory(new PropertyValueFactory<>(
                                                                    "OrderId"));
    productColumn.setCellValueFactory(new PropertyValueFactory<>("Product"));
    quantityColumn.setCellValueFactory(new PropertyValueFactory<>("Quantity"));
    supplierColumn.setCellValueFactory(new PropertyValueFactory<>("Product"));
    emissionDateColumn.setCellValueFactory(new PropertyValueFactory<>(
                                                              "EmissionDate"));
    isExpressColumn.setCellValueFactory(new PropertyValueFactory<>(
                                                                  "IsExpress"));
    extraWarrantyColumn.setCellValueFactory(new PropertyValueFactory<>(
                                                              "ExtraWarranty"));
  ...
  private void addButtonToTable() {
    ...
    Callback<TableColumn<Order, Void>, TableCell<Order, Void>> cellFactory =
      new Callback<>() {
        @Override
        public TableCell<Order, Void> call(
                                        final TableColumn<Order, Void> param) {
