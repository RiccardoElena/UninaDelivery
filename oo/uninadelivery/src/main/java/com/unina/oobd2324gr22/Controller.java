...
public class Controller {
  ...
  /**
   * On loginButton clicl, retrieves login data and checks if they are correct.
   * In such case, it loads the next page.
   *
  ...
  public final void loginButtonAction(final ActionEvent event) {
    ...
    if (!emailTextField.getText().isBlank()
      ...
      if (client == null) {
        ...
        FXMLLoader loader = new FXMLLoader(getClass()
                                            .getResource("Ordini.fxml"));
        Parent root;
        try {
          root = loader.load();
          OrdiniController controller = loader.getController();
          LocalDate bdate = LocalDate.parse("1999-12-31",
                                            DateTimeFormatter.ofPattern(
                                              "yyyy-MM-dd"));
          client = new Account("Gennaro", "De Gregorio",
                              "gdg@gmail.com", bdate, "gdg.jpg",
                          "password", null);
    ...
    this.showAlert(Alert.AlertType.ERROR, "Errore",
                      "Errore di login", errorMessage);
  }

  /* TODO!: we should consider making a custom alert class
  and moving this method there */
  private Optional<ButtonType> showAlert(final Alert.AlertType alertType,
  ...
  private Account checkLogin(final String email, final String password)
    throws Exception {
    try {
      return accountDAO.getAccountByEmailAndPassword(email,
                                                      new SHA256()
                                                        .toSHA256(password));
    } catch (Exception e) {
