...
public class ShipmentToClient extends Shipment {
  ...
  public ShipmentToClient(
      ...
      final WheeledSmall sTransport, final Deposit startDeposit) {
  ...
  /**
   * Implementation of the addOrder method for the ShipmentToClient class.
   * {@inheritDoc}
   */
   @Override
  public void addOrder(final Order order) {
    ...
    for (StoredProduct sp : this.getStartingDeposit().getStoredProducts()) {
      ...
      if (sp.getProduct().equals(order.getProduct())
          && sp.getQuantity() >= order.getQuantity()) {
        ...
        if (order
          ...
          throw new IllegalArgumentException(
            "The order is destined to another area");
        }
      }
    }
  }
