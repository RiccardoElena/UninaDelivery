...
\begin{lstlisting}[caption={Creazione della funzione \textbf{isAddressForSomethingSpecific\_func}}]
  ...
  $$
    DECLARE 
      -- Get the number of rows that match the given address
      flag integer;
      -- Search for the given address in the given table
      query text = 'SELECT COUNT(*) 
                    ...
                    WHERE AddressNo = NEW.AddressNo AND 
                          ...
                          Country = NEW.Country';
      -- temporary variable to store the table name
      tableName text;
  BEGIN
      -- Iterate through the arguments passed to the function
      FOREACH tableName IN ARRAY TG_ARGV LOOP
        -- If searching in deposit, func must have been called from account
        IF tableName == 'Deposit' THEN
          query = query || " AND DepositType <> 'City'";
        -- If searching in account, func must have been called from deposit
        ELSIF tableName == 'Account' THEN
          -- If the deposit is of type city, do nothing
          IF NEW.DepositType = 'City' THEN
            RETURN NEW;
          END IF;
        ELSE 
          -- entering this branch means that a wrong table name has been passed
