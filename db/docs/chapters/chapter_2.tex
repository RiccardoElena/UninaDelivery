...
% TODO @zGenny 
% [ ] update all diagram making contains a 1:N relationship and adding quantity to order. IsSent will disappear.
% [ ] remove all the reference to contains in dictonary (chapter 2). NOT ONLY BLINDLY DELETION CHECK IF SOMETHING BROKE
% [ ] remove all the reference to contains in the logic scheme (chapter 3) and change the mapping strategy to a foreign key migration
% [ ] remove contains from the db and the documentation of it (chapter 4). Tecnically you should simply need to drop the table and insert the 2 
% new column in order TRANSFERING ALL CONTRAINTS PRESENT IN CONTAINS FOR THE MOVED ROWS TO ORDER;
% [ ] implement isShipmentContainingValidOrders constraint in the db AND ADD THE PROCESS TO THE DOCUMENTATION (chapter 4)
% you can take as reference the constraint already present in the db and in the docs
...
\customTable{ccY}[Dizionario dei vincoli Quarta Parte]{\textbfB{Vincolo} & \textbfB{Tipologia} & \textbfB{Descrizione}}{
  ...
  %TODO zgenny modify this and make it a domain constraint as we discussed yesyerday. Already implemented in db
