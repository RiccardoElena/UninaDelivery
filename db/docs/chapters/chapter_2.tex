...
% FIXME: @zGenny there's a HUGE problem in the assosation between Zone and Address. The cardinality is wrong. It should be N to 1 and not N to N. We should discuss this asap.
\begin{center}
...
\customTable{cYYY}[Dizionario delle associazioni Prima Parte]{\textbfB{Associazione} & \textbfB{Descrizione} & \textbfB{Attributi} & \textbfB{Classi Coinvolte}}{
  ...
  % TODO: @zGenny probably 'In' is a better name for this association instead of 'Of'
  \textbf{In} &
  {\footnotesize
  Esprime la relazione tra \textbf{Zone} e \textbf{Address}. Un \textbf{Zone} può contenere più \textbf{Address}, e un \textbf{Address} può appartenere a più \textbf{Zone}.
  } &
  --- &
  {\footnotesize
  \textbf{Zone \([*]\)} (\textit{has}): Le zone che contengono un dato indirizzo;

  \textbf{Address \([*]\)} (\textit{in}): Gli indirizzi che appartengono a una data zona;
  }\\
  ...
  {\footnotesize
  \text{\textbf{IsSent}} (\textit{bool}) Indica se un dato prodotto in un dato ordine è stato spedito al cliente; 
...
\customTable{cYYY}[Dizionario delle associazioni Seconda Parte]{\textbfB{Associazione} & \textbfB{Descrizione} & \textbfB{Attributi} & \textbfB{Classi Coinvolte}}{
  ...
  % ? @zGenny we should consider to call this one 'Ships' insted of 'Contains' to avoid confusion with the other 'Contains' association 
  \textbf{Contains} &
  {\footnotesize
...
\customTable{cYYY}[Dizionario delle associazioni Terza Parte]{\textbfB{Associazione} & \textbfB{Descrizione} & \textbfB{Attributi} & \textbfB{Classi Coinvolte}}{
  ...
  \textbf{Covers} &
  {\footnotesize
  Esprime la relazione tra \textbf{Transport} e \textbf{Zone}. Un \textbf{Transport} può coprire più \textbf{Zone}, mentre una \textbf{Zone} può essere coperta da più \textbf{Transport}.
  } &
  {\footnotesize

  \textbf{Date} (\textit{date}): Data in cui un trasporto copre una certa zona. Un trasporto non può coprire più zone nella stessa data; 
  
  } &
  {\footnotesize
  \textbf{Zone \([*]\)} (\textit{is covered}): Le zone coperte da un dato mezzo di trasporto;
...
\customTable{ccY}[Dizionario dei vincoli Prima Parte]{\textbfB{Vincolo} & \textbfB{Tipologia} & \textbfB{Descrizione}}{
  ...
  {\footnotesize
  Il campo \textbf{Email} della classe \textbf{Account} deve avere il formato `\textbf{word}@\textbf{domain}.\textbf{ext}', dove \textit{word} può contenere solo caratteri alfanumerici, mentre \textit{domain} ed \textit{ext} solo caratteri alfabetici.
  ...
  % ? maybe this two should be considered n-ple type % TODO
  ...
  {\footnotesize
  Il campo \textbf{Businessmail} della classe Driver deve avere il formato `\textbf{n}.\textbf{surnameN}@uninadelivery.driver.com', dove \textit{n} è l'iniziale del nome dell'operatore, \textit{surname} è il suo cognome, mentre \textit{N} è un numero intero positivo, da utilizzare per eventuali omonimie.
  }\\
...
\customTable{ccY}[Dizionario dei vincoli Quinta Parte]{\textbfB{Vincolo} & \textbfB{Tipologia} & \textbfB{Descrizione}}{
  ...
  \textbf{isShipmentDirectedInCorrectZone} & Interrelazionale &
  {\footnotesize

  Il \textbf{Transport} associato a una \textbf{Shipment} deve coprire la \textbf{Zone} di destinazione della spedizione in data \textbf{ShippingDate}.
  La \textbf{Zone} di destinazione di una \textbf{Shipment} è la \textbf{Zone} dell'\textbf{Address} di un \textbf{Deposit} se la spedizione è diretta a un \textbf{Deposit}, altrimenti è la \textbf{Zone} dell'\textbf{Address} di tutti gli \textbf{Account} destinatari.
