...
\newpage % TODO: @RiccardoElena should this be removed?
...
\customTable{cYYY}[Dizionario delle associazioni Seconda Parte]{\textbfB{Associazione} & \textbfB{Descrizione} & \textbfB{Attributi} & \textbfB{Classi Coinvolte}}{
  ...
  \textbf{ShippedFrom} &
  {\footnotesize
  Esprime la relazione tra \textbf{Shipment} e \textbf{Deposit}. Una \textbf{Shipment} può partire da un solo \textbf{Deposit}, mentre un \textbf{Deposit} può essere il punto di partenza di più \textbf{Shipment}.
  } &
  --- &
  {\footnotesize
  ...
  --- &
  {\footnotesize
  \textbf{Shipment [*] (contains)}

  \textbf{Order [*] (is contained)}
  }\\
  ...
  --- &
  {\footnotesize
  \textbf{Operator [1] (creates)}

  \textbf{Shipment [*] (is created)}
  }\\
  ...
  --- &
  {\footnotesize
  \textbf{Shipment [*] (transportedBy)}

  \textbf{Transport [1] (transport)}
  }\\
  ...
  --- &
  {\footnotesize
  \textbf{Deposit [1] (belonged)}

  \textbf{Transport [*] (belongs)}
  }\\
}
\customTable{cYYY}[Dizionario delle associazioni Terza Parte]{\textbfB{Associazione} & \textbfB{Descrizione} & \textbfB{Attributi} & \textbfB{Classi Coinvolte}}{
  ...
  \textbf{Is} &
  {\footnotesize
  Esprime la relazione tra \textbf{Account} e \textbf{Driver}. Un \textbf{Account} potrebbe essere un \textbf{Driver}, mentre un \textbf{Driver} è un tipo di \textbf{Account}.
  } &
  --- &
  {\footnotesize
  \textbf{Account [1] (is)}

  \textbf{Driver [0..1] (is of)}
  }\\

  \textbf{Is} &
  {\footnotesize
  Esprime la relazione tra \textbf{Account} e \textbf{Operator}. Un \textbf{Account} potrebbe essere un \textbf{Operator}, mentre un \textbf{Operator} è un tipo di \textbf{Account}.
  } &
  --- &
  {\footnotesize
  \textbf{Account [1] (is)}

  \textbf{Operator [0..1] (is of)}
  }\\
  ...
  --- &
  {\footnotesize
  \textbf{Driver [0..*] (works At]}

  \textbf{Deposit [1] (works from)}
  }\\

  \textbf{Covers} &
  {\footnotesize
  Esprime la relazione tra \textbf{Transport} e \textbf{Zone}. Un \textbf{Transport} può coprire più \textbf{Zone} in una determinata \textbf{Date}, mentre una \textbf{Zone} può essere coperta da più \textbf{Transport}.
  } &
  \textbf{Date} &
  {\footnotesize
  \textbf{Zone [*] (is covered)}

  \textbf{Transport [*] (covers)}
  }\\

  \textbf{Drives} &
  {\footnotesize
  Esprime la relazione tra \textbf{Driver} e \textbf{Transport}. Un \textbf{Driver} può guidare più \textbf{Transport}, ma solo uno in una determinata \textbf{Date}, mentre un \textbf{Transport} può essere guidato da più \textbf{Driver}, ma soltanto da uno in una determinata \textbf{date}.
  
  } &
  \textbf{Date} &
  {\footnotesize
  \textbf{Driver [*] (Drives)}
  \textbf{Transport [*] (is driven)}
  }\\
}
...
\customTable{ccY}[Dizionario dei vincoli Seconda Parte]{\textbfB{Vincolo} & \textbfB{Tipologia} & \textbfB{Descrizione}}{
  ...
  }\\ %nota se arriva a 0 viene eliminato dal db
