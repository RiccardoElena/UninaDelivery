...
\begin{itemize}
  ...
  \item L'attributo \textbf{IsCompleted} dell'entità \textbf{Order}, che è stato conservato in quanto è un attributo che può essere richiesto frequentemente dal sistema, e richiede un interrogazione con risultato potenzialmente molto grande per essere valorizzato.
...
\customTable{cYYY}[Dizionario delle associazioni Seconda Parte]{\textbfB{Associazione} & \textbfB{Descrizione} & \textbfB{Attributi} & \textbfB{Classi Coinvolte}}{
  ...
  \textbf{Ships} &
  {\footnotesize
  Esprime la relazione tra \textbf{Shipment} e \textbf{Order}. Una \textbf{Shipment} può contenere più \textbf{Order}, e un \textbf{Order} può essere contenuto in più \textbf{Shipment}.
  } &
  --- &
  {\footnotesize
