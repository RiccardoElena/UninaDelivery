...
\customTable{ccY}[Dizionario dei vincoli Prima Parte]{\textbfB{Vincolo} & \textbfB{Tipologia} & \textbfB{Descrizione}}{
  ...
  {\footnotesize
  Il campo \textbf{Email} della classe \textbf{Account} deve avere il formato `\textbf{word}@\textbf{domain}.\textbf{ext}', dove \textit{word} può contenere solo caratteri alfanumerici, mentre \textit{domain} e \textit{ext} solo caratteri alfabetici.
  ...
  % ? maybe this two should be considered n-ple type
...
\customTable{ccY}[Dizionario dei vincoli Terza Parte]{\textbfB{Vincolo} & \textbfB{Tipologia} & \textbfB{Descrizione}}{
  ...
  {\footnotesize
  Un \textbf{Address}, non può non corrispondere né ad un \textbf{Deposit}, né a degli \textbf{Account}.
  }\\
