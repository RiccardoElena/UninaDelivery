...
% [ ]: add assosation class with Date field to drives association
% [ ]: to fix the "not world wide standard of zipcode problem" we can use both zipcode and country as primary key for the zone entity
% [ ]: add Price field to the Shipping entity as a calculated field. It will after removed in the restructuring phase
% [ ]: add the "isAvailable" field to the Driver entity
% [ ]: change the "OnTravel" field to "isAvailable" to the Vehicle entity for more felxibility
% [ ]: chenge the "IsCompleted" field to the Order entity to a calculated field. It can been extracted checking if all products in the order are in a shipment to the order maker. This one is gonna be stored in the restructured
% [ ]: add "HasArrived" field to the Shipment entity, to check if the shipment went to the destination or not
% [ ]: remove $FiscalCode" field to the Account entity, it's not needed
% [ ]: in the end export the new diagram as a pdf and replace the old one
